...
public class FileGridAdapter extends FileAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (v == null) {
			v = (LinearLayout) inflater.inflate(R.layout.gridfileitem, null,
					false);
		...
		/**
		 * if (fInfo.type != PKG) { if (fInfo.type == PHOTO){ Drawable d = new
		 * BitmapDrawable(fInfo.path());
		 * holder.getIcon(R.id.gridicon).setImageDrawable(d); }else
		 * holder.getIcon
		 * (R.id.gridicon).setImageBitmap(getIconBitmap(fInfo.type)); } else {
		 * Drawable d = fInfo.getAPKDrawable(packageManager); if (d != null) {
		 * holder.getIcon(R.id.gridicon).setImageDrawable(d); } else {
		 * holder.getIcon
		 * (R.id.gridicon).setImageBitmap(getIconBitmap(fInfo.type)); } } /
		 **/
