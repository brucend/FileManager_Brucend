...
import org.brucend.filemanager.util.XDialog.InputClick;
...
public class FileLibDialog extends Dialog implements DialogInterface {
	...
	public FileLibDialog(final FileManager fm, ArrayList<String> parent,
		...
		lists = (ExpandableListView) layout
				...
				if (doWhat == FILE_LIB_ADD) {
					...
					if (!fm.isMultFile()) {
						if (child.contains(path)) {
							Toast.makeText(fm, path + "已存在", Toast.LENGTH_SHORT)
									.show();
							return false;
						}
						child.add(path);
		...
		lists.setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view,
				...
				if (id < 0) {
					...
					b.setItems(cs, new AlertDialog.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							case 0:
								...
								b.setTitle("从库中删除")
										...
										.setNegativeButton(fm.cancel, cl)
										.create().show();
		...
		addBtn.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				...
				XDialog.inputDialog(fm, "添加文件库", "文件库名", null, null, fm.ok,
						fm.cancel, ic).show();
			}
		});
	...
	class ExpandListAdapter extends BaseExpandableListAdapter {
		...
		private View generateView(boolean parent, int groupPosition,
				int childPosition, ArrayList<String> child) {
