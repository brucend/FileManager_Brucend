...
import org.brucend.filemanager.CopyFileService.CopyFileBinder;
import org.brucend.filemanager.FileAdapter.FileInfo;
import org.brucend.filemanager.util.Common;
import org.brucend.filemanager.util.XDialog;
import org.brucend.filemanager.util.ZipUtils;
...
public class FileItemClickListener {
	 ...
	 * private String name;
	 * 
	 * public CopyWarnThread(String n) { // TODO Auto-generated constructor stub
	 * name = n; }
	 * 
	 ...
	 *           "文件名后面加(2)", "替换当前目录文件", "取消操作", "跳过", "以同样方式处理后面的文件"}, new
	 *           boolean[]{false, false, false, false, false}, new
	 *           AlertDialog.OnMultiChoiceClickListener(){
	 * @Override public void onClick(DialogInterface dialog, int which, boolean
	 *           isChecked) { // TODO Auto-generated method stub if (which != 4)
	 *           { copyWarningSelection = which; synchronized (fileManager){
	 *           fileManager.notify(); } dialog.dismiss(); } if (which == 4)
	 *           allDoLikeThis = isChecked; } }).create().show(); } } }; /
	 **/
	...
	public void onClick(View v, int which) {
		...
		case FileManager.MENU_ITEM_DELETE:
			if (fileManager.selectedItem().isEmpty()) {
				...
				doSelected = false;
				deleteDialog.setMessage(fileManager
						.getString(R.string.sure_delete_files) + name + "?");
				deleteDialog.show();
				break;
			}
			doSelected = true;
			deleteDialog.setMessage("确定删除  "
					+ fileManager.selectedItem().size() + " 个文件?");
			deleteDialog.show();
			break;
		case FileManager.MENU_ITEM_CREATE_DIRECTORY:
			createFileDialog.setTitle(fileManager
		...
		case FileManager.MENU_ITEM_RENAME:
			...
			renameDialog.setMessage(fileManager.getString(R.string.rename)
					+ name);
			renameDialog.show();
			break;
		...
		case FileManager.MENU_ITEM_OPEN:
			/**
			 * Log.d(tag, "zip: " + absPath); ZipUtils.zip(absPath, absPath +
			 * ".zip"); /
			 **/
			...
			case HANDLER_FILE_SIZE:
				if (perText != null && perString != null) {
					...
					perText.setText(perString + "大小:\t\t\t"
							+ Common.formatString(String.valueOf(size))
							+ " bytes\n");
				}
				break;
			}
	...
	private void dealCopyBack(int which) {
		...
		Toast.makeText(fileManager, result, Toast.LENGTH_SHORT).show();
		if (pastePath.equals(fileManager.currentPath())) {
			fileManager.listViewHandler
					.sendEmptyMessage(FileManager.HANDLER_REFRESH_LISTVIEW);
		}
		if (isCut)
			clearPaste();
		isCopying = false;
	...
	private void initCutProgressDialog() {
		...
		cutProgressDialog
				.setTitle(fileManager.getString(R.string.moving_files));
		cutProgressDialog.setCancelable(false);
		cutProgressDialog.setButton(ProgressDialog.BUTTON_NEGATIVE,
				fileManager.cancel, new ProgressDialog.OnClickListener() {
	...
	DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			case ProgressDialog.BUTTON_NEGATIVE:
				copyFileService.cancelCopy();
				Toast.makeText(fileManager, "正在取消复制", Toast.LENGTH_SHORT)
						.show();
				dialog.dismiss();
				isCopying = false;
				// Hide
	...
	private void initCopyProgressDialog() {
		copyProgressDialog = new ProgressDialog(fileManager); // ,
																// R.style.dialog);
																// //,
		...
		copyProgressDialog.setTitle(fileManager.getString(R.string.copyFile));
	...
	private void showCopyWarnDialog(String name) {
		synchronized (this) {
			String[] items = fileManager.getResources().getStringArray(
					R.array.copyWarning);
			copyWarningDialog.setTitle(name
	 ...
	 * synchronized (this){ String[] items =
	 * fileManager.getResources().getStringArray(R.array.copyWarning);
	 * copyWarningDialog.setTitle(name +
	 * fileManager.getString(R.string.existed));
	 * copyWarningDialog.setMultiChoiceItems(items, new boolean[]{false, false,
	 * false, false, false}, new AlertDialog.OnMultiChoiceClickListener(){
	 * 
	 * @Override public void onClick(DialogInterface dialog, int which, boolean
	 *           isChecked) { // TODO Auto-generated method stub if (which != 4)
	 *           { copyFileService.selection = which; synchronized
	 *           (fileManager){ fileManager.notify(); } dialog.dismiss(); if
	 *           (warn != null) warn.clickCallback(which, isChecked); } if
	 *           (which == 4) copyFileService.allDoSame = isChecked; }
	 *           }).create().show(); } } /
	 **/
	...
	private void init() {
		...
		renameDialog = b
				.setTitle(fileManager.getString(R.string.rename))
				.setPositiveButton(fileManager.ok,
						new AlertDialog.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								renamePath = fileManager.currentPath() + "/"
										+ newName;
	...
	private void initSpinner() {
		...
		dView = (ScrollView) inflater.inflate(R.layout.filepermission, null);
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				try {
					...
					if (fileManager.isRoot()) {
						...
						p = fileManager.linux.shell.exec(String.format(
								"chmod %s %x ", all, per) + absPath);
					}
					int w = p.waitFor();
		...
		b.setPositiveButton(fileManager.ok, lt)
				.setNegativeButton(fileManager.cancel, lt).setCancelable(false);
	...
	/**
	 * 获取文件的基本属性,并显示 null 0 0 read 4 1 write 6 2 readE 5 3 writeE 7 4
	 * */
	 ...
	 * 用root权限拷贝 *
	 * 
	 * Process rootProgress = null; boolean rootCopying = false; public void
	 * rootCopy(){ Runnable r = new Runnable() {
	 * 
	 * @Override public void run() { // TODO Auto-generated method stub
	 *           DataOutputStream shell; BufferedReader err;
	 * 
	 *           if (multPaste.isEmpty()) { Log.d(tag, "empty"); return; }
	 *           rootCopying = true; isCopying = true; int size =
	 *           multPaste.size(); try { rootProgress =
	 *           Runtime.getRuntime().exec("su"); shell = new
	 *           DataOutputStream(rootProgress.getOutputStream()); String cmd =
	 *           " " ; String fPath = "";
	 * 
	 *           err = new BufferedReader(new InputStreamReader
	 *           (rootProgress.getErrorStream()));
	 * 
	 *           File dst = null; String dstFile = null; String dstName = null;
	 *           String currentPath = fileManager.currentPath(); for (int i = 0;
	 *           i < size; i++) { fPath = multPaste.get(i); dstName =
	 *           Common.getPathName(fPath); dstFile = currentPath + "/" +
	 *           dstName; dst = new File(dstFile); if (dst.exists()){
	 *           synchronized (fileManager){ if (!copyFileService.allDoSame){
	 *           Message msg = xHandler.obtainMessage( HANDLER_SHOW_COPY_WARN);
	 *           Bundle bundle = new Bundle();
	 *           bundle.putString(BUNDLE_MULT_FILE_PATH, dstName);
	 *           bundle.putBoolean(BUNDLE_IS_FILE, dst.isFile());
	 *           msg.setData(bundle); xHandler.sendMessage(msg);
	 *           fileManager.wait(); }
	 * 
	 *           switch (copyFileService.selection) { case COPY_APPEND: dstFile
	 *           = Common.fileNameAppend(dstFile, "(2)"); break;
	 * 
	 *           case COPY_REPLACED: break;
	 * 
	 *           case COPY_SKIP: continue;
	 * 
	 *           case COPY_CANCEL: default: rootProgress.destroy(); return ; } }
	 *           }
	 * 
	 *           cmd += "cp -r \"" + fPath + "\" \"" + dstFile + "\"\n"; }
	 *           Log.d(tag, cmd); cmd += "exit\n"; shell.write(cmd.getBytes());
	 *           shell.flush(); if (isCut) multPaste.clear();
	 *           //Log.d("FileDialog", "re: " + rootProgress.waitFor()); if
	 *           (err.ready()){ Log.d(tag, "" + err.readLine()); } } catch
	 *           (FileNotFoundException e) { // TODO Auto-generated catch block
	 *           e.printStackTrace(); } catch (IOException e) { // TODO
	 *           Auto-generated catch block e.printStackTrace(); } catch
	 *           (InterruptedException e) { // TODO Auto-generated catch block
	 *           e.printStackTrace(); }finally{ isCopying = false; rootCopying =
	 *           false; } } }; new Thread(r).start(); }
	 * 
	 * 
	 *           /
	 **/
		...
		public void run() {
			if (!fileManager.isMultFile()) {
				...
				for (int i = 0; i < size; i++) {
					selString.add(fis.get(fileManager.selectedItem().get(i))
							.path());
				}
	...
	private void doCompressZip() {
		XDialog.InputClick click = new XDialog.InputClick() {
			...
			public void onClickListener(String str, int which) {
				...
				AlertDialog.Builder b = new AlertDialog.Builder(fileManager);
				b.setView(
						fileManager.getLayoutInflater().inflate(
								R.layout.cziplayout, null))
						.setCancelable(false);
		...
		XDialog.inputDialog((Context) fileManager, "压缩zip",
				"压缩 " + absPath + " 到", absPath + ".zip", null, fileManager.ok,
				fileManager.cancel, click).show();
	...
	private void doDecompressZip() {
		...
		XDialog.inputDialog(fileManager, "解压", "解压 " + absPath + " 到", name,
				null, fileManager.ok, fileManager.cancel, click).show();
	}
	...
	private void doRename() {
		...
		try {
			if (fileManager.isRoot() == false) {
				...
				DataOutputStream shell = new DataOutputStream(
						p.getOutputStream());
				String cmd = "mv \"" + absPath + "\" \"" + renamePath
						+ "\"\nexit\n";
				shell.write(cmd.getBytes());
				shell.flush();
				shell.close();
			...
			if (p.waitFor() != 0) {
				...
				/**
				 * msg.arg1 = FileDialog.HANDLER_SET_LISTVIEW_SELECTED; msg.arg2
				 * = position; /
				 **/
	...
	private void initDeleDialog() {
		AlertDialog.Builder b = new AlertDialog.Builder(fileManager);
		deleteDialog = b
				.setTitle(fileManager.getString(R.string.dele_file))
				.setPositiveButton(fileManager.ok,
						new AlertDialog.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								doDeleDialog.setMessage(fileManager
										.getString(R.string.dele_file));
								doDeleDialog
										.setProgressStyle(ProgressDialog.STYLE_SPINNER);
								doDeleDialog.setButton(
		...
		try {
			if (fileManager.isRoot()) {
				...
				if (fileManager.multFile && doSelected) {
					selString.add(0, "rm");
					selString.add(1, "-r");
					deleProgress = fileManager.linux.shell.exec(selString
							.toArray(new String[size + 2]));
				} else {
	...
	public void doOpenFile(String file) {
		...
		/**
		 * Log.d(tag, "zip: " + file); ZipUtils.zip(file, file + "2"); /
		 **/
		/***/
	...
	public void doPaste() {
		...
		/**
		 * int ret = checkMultFile(from, to); if (ret == -1 || ret ==
		 * COPY_CANCEL) return; /*** if (fileManager.currentPath().equals("/")){
		 * File toFile = new File(fileManager.currentPath()); if
		 * (!toFile.canWrite()) { Toast.makeText(fileManager,
		 * "不支持root复制到根目录('/')", Toast.LENGTH_LONG).show(); return; } } /
		 ***/
	...
	private final void doBindCopyService() {
		...
		fileManager.bindService(new Intent(fileManager, CopyFileService.class),
				xSConnection, Context.BIND_AUTO_CREATE);
	}
	...
	void startCopyService(ArrayList<String> from, String to, boolean cut) {
		/**
		 * if (copyFileService.isDestroyed()) { doBindCopyService(); } if
		 * (copyFileService != null) copyFileService.startCopy(from, to); else
		 * Log.d(tag, "==null"); /**
		 * 
		 * Process p = null; BufferedReader ber = null; try {
		 * 
		 * p = fileManager.linux.shell.exec("su"); DataOutputStream shell = new
		 * DataOutputStream(p.getOutputStream()); String cmd = "ls -l -a \"" +
		 * to + "/..\"\nexit\n"; shell.write(cmd.getBytes()); shell.flush();
		 * shell.close(); br = new BufferedReader( new
		 * InputStreamReader(p.getErrorStream())); ber = new BufferedReader( new
		 * InputStreamReader(p.getInputStream())); int w = p.waitFor(); //String
		 * con = ber.readLine(); if (w != 0){ Toast.makeText(fileManager,
		 * br.readLine(), Toast.LENGTH_LONG).show(); br.close(); ber.close();
		 * return; }
		 * 
		 * //从shell中获取内容 String con; while ((con = ber.readLine()) != null){ if
		 * (con.endsWith(Common.getPathName2(to))){ break; } } if (con == null)
		 ...
		 * Auto-generated catch block e.printStackTrace(); } finally { if (p !=
		 * null) p.destroy(); if (br != null) { try { br.close(); if (ber !=
		 * null) ber.close(); } catch (IOException e) { // TODO Auto-generated
		...
		fileManager
				.startService(new Intent(fileManager, CopyFileService.class));
		/**/
	}
	...
	public void cancelCopy() {
		...
		isCopying = false;
		/**
		 * if (rootCopying){ if (rootProgress != null) rootProgress.destroy();
		 * return; } /
		 **/
		copyFileService.cancelCopy();
	}
	...
	/**
	 * 根据粘贴板 fileDialog.paste 的内容, 是否有文件名同当前文件相同. 相同则弹出对话框由用户决定, 重命名, 覆盖, 取消,
	 * 路过之类.
	 * 
	 * @return 1 正常返回,已筛选出要复制的文件名, 2(COPY_CANCEL) 返回 取消 复制文件 -1 出错
	 * */

	private void initRenameWarningDialog() {
		AlertDialog.Builder b = new AlertDialog.Builder(fileManager);
		b.setTitle(fileManager.getString(R.string.rename))
				.setItems(
						fileManager.getResources().getStringArray(
								R.array.rename_mult),
						new AlertDialog.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog,
								...
								switch (which) {
								case 0: // append (2)
									renamePath = Common.fileNameAppend(
											renamePath, "(2)");
									doRename();
									break;
								case 1: // replace
				...
				.setPositiveButton(fileManager.ok,
						new AlertDialog.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								String cFile = fileManager.currentPath() + "/"
										+ n;
										...
										if (createDirectory) {
											if (!(success = mFile.mkdirs()))
												Toast.makeText(
														fileManager,
														fileManager
																.getString(R.string.creat_folder_failth),
														Toast.LENGTH_SHORT)
														.show();
										} else {
											if (!(success = mFile
												...
												Toast.makeText(
														fileManager,
														fileManager
																.getString(R.string.creat_file_failth),
														Toast.LENGTH_SHORT)
														.show();
										}
									}
									if (success) {
										fileManager.refreshPath(
												fileManager.currentPath(), 0);
										fileManager.fileViewList
												.setSelection(fileManager.listListener.position);
									}
								} catch (IOException e) {
									e.printStackTrace();
								} catch (InterruptedException e) {
	...
	private void initOpenMannerDialog() {
		AlertDialog.Builder b = new AlertDialog.Builder(fileManager);
		openMannerDialog = b
				.setTitle(fileManager.getString(R.string.choice_program))
				.setItems(
						fileManager.getResources().getStringArray(
								R.array.programs),
						new AlertDialog.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								try {
									fileManager.startActivityForResult(intent,
											2);
								} catch (ActivityNotFoundException e) {
									Toast.makeText(
											fileManager,
											fileManager
													.getString(R.string.can_not_open_file),
											Toast.LENGTH_SHORT).show();
								}
							}
						})
